...
class Particle {
    public:
    Particle (const char* iName, double iPx, double iPy, double iPz) : fPx{iPx}, fPy{iPy}, fPz{iPz} {
        fIParticle = FindParticle(iName);
        if (fIParticle==fNParticleType) std::cout << "Particle " << iName << " is not in the list. Add it trough the \"AddParticleType\" method. " << '\n';
    }
...
//    Static Methods
    ...
    static void AddParticleType (const char* iName, double iMass, int iCharge, double iWidth = 0);
