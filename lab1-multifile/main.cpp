...
//    creating Histograms
    ...
    TH1F *hAzimuthal = new TH1F("hAzimuthal", "Azimuthal angle distribution", 500, 0, 2 * M_PI);
    TH1F *hPolar = new TH1F("hPolar", "Polar angle distribution", 500, 0, 2 * M_PI);
    TH1F *hMomentum = new TH1F ("hMomentum", "Momentum", nBins, 0, 5.5);
    TH1F *hTransverseMomentum = new TH1F ("hTransverseMomentum", "Transverse momentum", nBins, 0, 4);
    TH1F *hEnergy = new TH1F ("hEnergy", "Particles' energy", nBins, 0, 5);
    TH1F *hInvMass = new TH1F ("hInvMass", "Mass Invariant", nBins, 0, 5);
    TH1F *hInvMassConcordant = new TH1F ("hInvMassConcordant", "Mass Invariant - Concordant Particles", nBins, 0, 5);
...
//        Generating the i-particle following the given proportions and filling hProportions
        double proportions = gRandom->Rndm();
        if (proportions<0.4) { // 80% pions
             particles[i].SetParticle(p_p); hProportions->Fill(0);
        } else if (proportions < 0.8){
            particles[i].SetParticle(p_m); hProportions->Fill(1);
        } else if (proportions < 0.85) {
            particles[i].SetParticle(k_p); hProportions->Fill(2);
        } else if (proportions< 0.9) { //10% kaons
            particles[i].SetParticle(k_m); hProportions->Fill(3);
        } else if (proportions<0.945) {
            particles[i].SetParticle(pr_p); hProportions->Fill(4);
        } else if (proportions<0.99) { //9% protons
            particles[i].SetParticle(pr_m); hProportions->Fill(5);
        } else if (proportions<0.995){
           particles[i].SetParticle(Kstar); hProportions->Fill(6);
           particles[100+array_tail].SetParticle(p_p); ++array_tail;
           particles[100+array_tail].SetParticle(k_m); ++array_tail;
           particles[i].Decay2body(particles[99+array_tail], particles[98+array_tail]);
           hInvMassKstarDecay ->Fill(particles[99+array_tail].InvMass(particles[98+array_tail]));
        } else { // 1% - K* decays 50% in p+,k- and 50% in p-,k+
            particles[i].SetParticle(Kstar); hProportions->Fill(6);
            particles[100+array_tail].SetParticle(p_m); ++array_tail;
            particles[100+array_tail].SetParticle(k_p); ++array_tail;
            particles[i].Decay2body(particles[99+array_tail], particles[98+array_tail]);
            hInvMassKstarDecay ->Fill(particles[99+array_tail].InvMass(particles[98+array_tail]));
        }
...
//  Filling the Mass Invariant's histograms
        for (int k=0; k!=100+array_tail-1; ++k){
            for (int t=k+1; t!=100+array_tail; ++t){
                ...
                bool p_k = (iMass == pMass && jMass == kMass) || (iMass == kMass && jMass == pMass) ;
                ...
                if (particles[k].getParticleCharge()>0) {
                    if (particles[t].getParticleCharge()>0) {
                        ...
                        if ( p_k ) { hInvMassPKconcordant -> Fill(mass_invariant); }
                    }
                    else if (particles[t].getParticleCharge()<0) {
                        hInvMassDiscordant -> Fill(mass_invariant);
                        if ( p_k ) { hInvMassPKdiscordant -> Fill(mass_invariant); }
                    }
