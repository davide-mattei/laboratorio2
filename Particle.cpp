...
int Particle::getIparticle () const {
    return fIParticle;
}
...
double Particle::InvMass(Particle const& p) const {
    return sqrt(pow(p.getParticleEnergy()+this->getParticleEnergy(),2)+pow(p.getTotalMomentum()+this->getTotalMomentum(),2));
}
...
void Particle::SetParticle (char* iName) {
    ...
    if ( index == fNParticleType) {
        std::cout << "Particle is not in the array. Add it through the AddParticleType method." << '\n';
    } else {
        fIParticle = index;
    }
}
...
void Particle::AddParticleType (const char* iName, double iMass, int iCharge, double iWidth) {
    ...
    } else if (iWidth == 0) { fParticleType[fNParticleType] = new ParticleType{iName, iMass, iCharge}; ++fNParticleType;
    } else { fParticleType[fNParticleType] = new ResonanceType{iName, iMass, iCharge, iWidth};
        ++fNParticleType;
    }
}
...
int Particle::FindParticle (const char* iName) {
    for(int i=0; i!=fNParticleType; ++i){
        if (iName == fParticleType[i]->getName()) {return i;}
    }
